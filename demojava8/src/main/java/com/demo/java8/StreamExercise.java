...
public class StreamExercise {
  public static void main(String[] args) {
    ...
    List<Integer> result3 = words.stream()
    ...
    List<Integer> numbers3 = Arrays.asList(5, 10, 15, 20, 10, 5);
        ...
        // 2. Filter for numbers > 10
        // 3. Collect into a Set (automatically handles uniqueness)
        Set<Integer> result4 = numbers3.stream()
                                      .filter(n -> n > 10)
    ...
    List<Student> students = Arrays.asList(
        ...
        Map<String, Integer> result3Map = students.stream()
    ...
    List<Employee> employees = Arrays.asList(
        ...
        // Logic: Filter by salary, then extract names
        List<String> highEarners = employees.stream()
            .filter(emp -> emp.getSalary() >= 50000) // Keep those with 50k+
            .map(Employee::getName)                  // Get only the name string
            .collect(Collectors.toList());           // Wrap it back into a List
    ...
    List<Person> people = Arrays.asList(
        ...
        Map<Integer, List<String>> namesByAge = people.stream()
            .collect(Collectors.groupingBy(
                Person::getAge,                      // Key: The Age
                Collectors.mapping(                  // Value: Transform the list
                    Person::getName,                 // Extract the name
                    Collectors.toList()              // Collect names into a List
                )
            ));

        System.out.println(namesByAge);
        // Output: {25=[Bob], 30=[Alice, Charlie]}
    ...
    List<Children> childrenList = Arrays.asList(
            new Children("Alice", 45),
            new Children("Bob", 55),
            new Children("Charlie", 40),
            new Children("David", 70)
    ...
    List<String> languages = Arrays.asList("Java", "Python", "Rust", "R", "Go");
        ...
        // 2. Use the joining collector with a delimiter
        String result0 = languages.stream()
    ...
    // Output: [4, 5, 6, 7, 8]
    elements.stream().sorted().skip(3L).limit(5L).collect(Collectors.toList());
    

   // List<Integer> ages = Arrays.asList(4, 7, 9, 12, 16, 21);

        // 1. Stream the integers
        // 2. Filter for numbers divisible by 3 (n % 3 == 0)
        // 3. Get the first matching result
        Optional<Integer> firstDivisibleByThree = ages.stream()
            .filter(n -> n % 3 == 0)
            .findFirst();
    ...
    List<Integer> amounts = Arrays.asList(1, 2, 3, 4);
        ...
        List<Integer> result24 = amounts.stream()
            .map(n -> n * 2)             
            .peek(n -> System.out.println("Intermediate: " + n))
            .collect(Collectors.toList());

        System.out.println("Final Output: " + result24);
    ...
    List<String> animals = Arrays.asList("cat", "tiger", "panda", "dog");
        ...
        // 1. Convert to stream
        // 2. Filter for strings with length > 4
        // 3. Find the first occurrence
        Optional<String> result25 = animals.stream()
                                         .filter(s -> s.length() > 4)
                                         .findFirst();
...
List<Integer> duplicates = Arrays.asList(2, 1, 2, 3, 4, 3, 5, 5, 6);
        ...
        // Custom Collector definition
        Collector<Integer, Set<Integer>, Set<Integer>> toCustomSet = Collector.of(
            HashSet::new,               // Supplier: creates the container
            Set::add,                   // Accumulator: adds element to container
            (left, right) -> {          // Combiner: merges two containers (for parallel)
                left.addAll(right); 
                return left; 
            },
            Collector.Characteristics.IDENTITY_FINISH // Finisher: result is the container itself
        );

        Set<Integer> result26 = duplicates.stream().collect(toCustomSet);
